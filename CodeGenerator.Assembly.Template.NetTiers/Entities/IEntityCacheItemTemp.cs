...
{
    public class IEntityCacheItemTemp :  CodeGeneratorBase<EntitiesFactory, DatabaseInfoModel>
    {
        private async Task<CompilationUnitSyntax> GetCompilationUnit(DatabaseInfoModel model, string className)
        {
            var tree = CSharpSyntaxTree.ParseText(StaticFileResource.IEntityCacheItem);
        ...
        {
            string className = "IEntityCacheItem";
