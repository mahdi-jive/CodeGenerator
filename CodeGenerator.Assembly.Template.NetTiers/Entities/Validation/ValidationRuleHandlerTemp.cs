...
namespace CodeGenerator.Assembly.Template.NetTiers.Entities.Validation
{
    internal class ValidationRuleHandlerTemp : CodeGeneratorBase<ValidationDirectory, DatabaseInfoModel>
    {
        private async Task<CompilationUnitSyntax> GetCompilationUnit(DatabaseInfoModel model, string className)
        {
            var tree = CSharpSyntaxTree.ParseText(StaticFileResource.ValidationRuleHandler);
        ...
        {
            string className = "ValidationRuleHandler";
