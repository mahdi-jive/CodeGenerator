...
{
    public class IEntityTemp :  CodeGeneratorBase<EntitiesFactory, DatabaseInfoModel>
    {
        private async Task<CompilationUnitSyntax> GetCompilationUnit(DatabaseInfoModel model, string className)
        {
            var tree = CSharpSyntaxTree.ParseText(StaticFileResource.IEntity);
        ...
        {
            string className = "IEntity";
            var model = contextModel as DatabaseInfoModel;
            CompilationUnitSyntax compilationUnit = await GetCompilationUnit(model, className);
            List<ICodeFile> codeFiles = new List<ICodeFile>() { new CodeFile($"{className}.cs", className, compilationUnit) };
            return codeFiles;
        }
    }
}
