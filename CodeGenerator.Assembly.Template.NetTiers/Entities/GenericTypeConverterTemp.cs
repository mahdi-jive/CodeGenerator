...
{
    public class GenericTypeConverterTemp :  CodeGeneratorBase<EntitiesFactory, DatabaseInfoModel>
    {
        private async Task<CompilationUnitSyntax> GetCompilationUnit(DatabaseInfoModel model, string className)
        {
            var tree = CSharpSyntaxTree.ParseText(StaticFileResource.GenericTypeConverter);
        ...
        {
            string className = "GenericTypeConverter";
