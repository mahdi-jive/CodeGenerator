...
{
    internal class ViewProviderGeneratedTemp : CodeGeneratorBase<ViewsDirectory, DatabaseInfoModel>
    {
        public override async Task<IEnumerable<ICodeFile>> Generate(ITemplateRenderer renderer, DatabaseInfoModel contextModel)
        {
            ...
            if (model != null)
            {
                foreach (var item in await model.Views)
                {
                    try
                    {
                        var tempModel = await ViewProviderGeneratedTempModel.CreateModel(item);
                        var compilationUnit = await renderer.RenderAsync("SqlClient/Views/ViewProviderGeneratedTemp.cshtml", tempModel);
